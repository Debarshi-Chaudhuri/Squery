...
class QuesList extends React.Component{
    ...
    componentDidMount()
    {
        ...
        db.collection('answeredques').doc(this.props.uname).collection('qna').get().then((query)=>query.forEach((doc)=>this.setState({ques:this.state.ques.concat(doc.data())})),this.setState({bool:true}))
    ...
    render()
    {
     ...
     // db.collection('answeredques').doc('Pacharjee').collection('qna').get().then((doc)=>doc.forEach((d)=>d.data().answer.get().then((e)=>{console.log(e.data())})))
            ...
            <h1>{h1}</h1>
            <div>{
                this.state.bool&&this.state.ques.map((items)=><QuesItem items={items} uname={this.props.uname}/>)
            }</div>
        </div>
        );
    }
